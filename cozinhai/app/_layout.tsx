...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadStorageData = async () => {
    try {
      ...
      if (storedToken && storedUser) {
        ...
        console.log('Usuário carregado do storage:', JSON.parse(storedUser).name);
      } else {
  ...
  const signIn = async (email: string, password: string) => {
    try {
      ...
      const tokenString = typeof authData.access_token === 'string' 
        ? authData.access_token 
        : authData.access_token?.access_token || JSON.stringify(authData.access_token);
      ...
      if (error.name === 'AbortError') {
        throw new Error('Tempo esgotado. O servidor pode estar iniciando, aguarde 1 minuto e tente novamente.');
      }
...
function AuthGate() {
  ...
  useEffect(() => {
    ...
    // Se ESTÁ logado e está em /login ou /register → redireciona para tabs
      ...
      console.log('Usuário logado em rota pública, redirecionando para /(tabs)');
      router.replace('/(tabs)');
    }
